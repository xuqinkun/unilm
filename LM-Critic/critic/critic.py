...
def run_gpt2(sents, cuda=True, model_name=None):
    ...
    loss = get_gpt2_loss(input_ids=inputs['input_ids'], attention_mask=inputs['attention_mask'],
...
def gpt2_critic_char_level_only(sent, verbose=1, cuda=True, fp16=True, seed='auto', n_samples=100):
    ...
    if verbose:
        if is_good:
            ...
            print(
                'Neighbor sentence with highest log(p): {} (= {:.3f})'.format(sents[best_idx], float(logps[best_idx])))
